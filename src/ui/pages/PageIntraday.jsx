      ...
      {showLock ? (
        <Card title={tSafe(lang, "market.closed", L("장 마감", "Market Closed"))} subtitle={tSafe(lang, "a2.lockSub", L("장 외 시간에는 인트라데이 신호가 비활성화됩니다.", "Intraday is disabled off-hours"))}>
          ...
          <div className="mt-2 text-[11px] text-white/55">
            {tSafe(lang, "market.noteDaily", "Daily interpretation remains available off-hours.")}
          </div>
        </Card>
      ) : (
        <div className="grid gap-3">
          <Card title={tSafe(lang, "a2.title", "Intraday Diagnostics")} subtitle={tSafe(lang, "a2.subtitle", "Deviation watch vs daily scenario")}>
            <div className="flex items-center gap-2">
              {diag.deviation ? (
                <Pill tone="yellow" label={tSafe(lang, "a2.deviation", "Deviation")} msg={tSafe(lang, "a2.devMsg", "Intraday signals deviate from baseline. Avoid single-scenario calls.")} lang={lang} />
              ) : (
                <Pill tone="gray" label={tSafe(lang, "a2.normal", "Within range")} msg={tSafe(lang, "a2.normMsg", "No strong intraday dislocation flags detected.")} lang={lang} />
              )}
              {diag.corrSurge ? <Pill tone="yellow" label="corrSurge" msg={tSafe(lang, "a2.corrMsg", "Cross-asset correlation is spiking.")} lang={lang} /> : null}
            </div>

            {diag.reasons?.length ? (
              <div className="mt-2 text-xs text-white/60 leading-snug">
                {tSafe(lang, "a2.reasons", "Reasons")}: {diag.reasons.join(" · ")}
              </div>
            ) : null}
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-white/75">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-2">
                <div className="text-[11px] text-white/55">{tSafe(lang, "a2.refresh", "Refresh")}</div>
